...
@testset "Bootstrapping and PDEs" begin
    ...
    @test initialCondition_all[:, 1] == initialCondition_all[:, 10] == initialCondition_all[:, 10]
    ...
    initialCondition_all = EquationLearning.compute_initial_conditions(x_pde, t_pde, u_pde, bgp1, ICType)
    @test size(initialCondition_all) == (N, B)
    @test initialCondition_all[391, 4] ≈ 46.222676778665665
    @test initialCondition_all[[1, 2, 5, 15, 100, 99, 500, 399, 4], 1] ≈ [304.66050973379447,
        302.2010450384069,
        294.8226509522441,
        273.07356736917706,
        239.48588164575213,
        ...
        85.81701188374014,
        297.2821156476317]
    @test initialCondition_all[[1, 100, 400, 499, 2, 3, 50], 7] ≈ [274.46119525381624,
        237.6837607718735,
        86.1642364390944,
        361.0936371628274,
        274.57025177005727,
