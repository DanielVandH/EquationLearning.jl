...
for (k, (i, j)) in enumerate(plot_cart_idx)
    data = assay_data[k]
    assay_plots[i, j] = Axis(jin_assay_data_fig[i, j], xlabel=L"$x$ (μm)", ylabel=L"u(x, t)/K",
        title="($(alphabet[k])): $(10+2*(k-1)),000 cells per well",
        titlealign=:left)
    for (s, T) in enumerate(unique(data.Time))
        scatter!(assay_plots[i, j], data.Position[data.Time.==T] * x_scale, data.Dens1[data.Time.==T] / x_scale^2 / unscaled_K, color=colors[s], markersize=3)
        scatter!(assay_plots[i, j], data.Position[data.Time.==T] * x_scale, data.Dens2[data.Time.==T] / x_scale^2 / unscaled_K, color=colors[s], markersize=3)
        scatter!(assay_plots[i, j], data.Position[data.Time.==T] * x_scale, data.Dens3[data.Time.==T] / x_scale^2 / unscaled_K, color=colors[s], markersize=3)
        lines!(assay_plots[i, j], data.Position[data.Time.==T] * x_scale, data.AvgDens[data.Time.==T] / x_scale^2 / unscaled_K, color=colors[s])
    end
    hlines!(assay_plots[i, j], K / x_scale^2 / unscaled_K, color=:black)
    ylims!(assay_plots[i, j], 0.0, 1.3)
end
...
for (k, (i, j)) in enumerate(plot_cart_idx)
    ...
    lower = μ .- 2sqrt.(Σ)
    upper = μ .+ 2sqrt.(Σ)
    gp_plots[i, j] = Axis(jin_assay_data_gp_bands_fig[i, j], xlabel=L"$x$ (μm)", ylabel=L"u(x, t)/K",
        title="($(alphabet[k])): $(10+2*(k-1)),000 cells per well",
        titlealign=:left)
    for (s, T) in enumerate(unique(t))
        scatter!(gp_plots[i, j], x[t.==T] * x_scale, u[t.==T] / x_scale^2 / unscaled_K, color=colors[s], markersize=3)
        lines!(gp_plots[i, j], data.Position[data.Time.==T] * x_scale, μ[data.Time.==T] / x_scale^2 / unscaled_K, color=colors[s])
        band!(gp_plots[i, j], data.Position[data.Time.==T] * x_scale, upper[data.Time.==T] / x_scale^2 / unscaled_K, lower[data.Time.==T] / x_scale^2 / unscaled_K, color=(colors[s], 0.35))
    end
    hlines!(gp_plots[i, j], K / x_scale^2 / unscaled_K, color=:black)
    ylims!(gp_plots[i, j], 0.0, 1.3)
end
...
function plot_fisher_kolmogorov_delay(bgp::BootResults, x_scale, t_scale, filename, colors, dat_idx, assay_data, fontsize, unscaled_K)
    ...
    M = length(bgp.pde_setup.δt)
    GPAxis = Axis(resultFigures[3, 2], xlabel=L"$x$ (μm)", ylabel=L"u(x, t)/K", title=@sprintf("(h): PDE (Sampled IC).\nError: (%.4g, %.4g)", err_CI[1], err_CI[2]), titlealign=:left)
    @views for j in 1:M
...
function plot_generalised_fkpp_delay(bgp::BootResults, x_scale, t_scale, filename, colors, dat_idx, assay_data, fontsize, unscaled_K)
    dat = assay_data[dat_idx]
    x_pde = dat.Position
    t_pde = dat.Time
    u_pde = dat.AvgDens
    ...
    err_CI = error_comp(bgp, pde_gp, x_pde, t_pde, u_pde)
    M = length(bgp.pde_setup.δt)
    GPAxis = Axis(resultFigures[5, 2], xlabel=L"$x$ (μm)", ylabel=L"u(x, t)/K", title=@sprintf("(j): PDE (Sampled IC). Error: (%.4g, %.4g)", err_CI[1], err_CI[2]), titlealign=:left)
    @views for j in 1:M
        lines!(GPAxis, bgp.pde_setup.meshPoints * x_scale, soln_vals_mean[:, j] / x_scale^2 / unscaled_K, color=colors[j])
        band!(GPAxis, bgp.pde_setup.meshPoints * x_scale, soln_vals_upper[:, j] / x_scale^2 / unscaled_K, soln_vals_lower[:, j] / x_scale^2 / unscaled_K, color=(colors[j], 0.35))
        CairoMakie.scatter!(GPAxis, x_pde[t_pde.==bgp.pde_setup.δt[j]] * x_scale, u_pde[t_pde.==bgp.pde_setup.δt[j]] / x_scale^2 / unscaled_K, color=colors[j], markersize=3)
...
function plot_pde_soln!(fig, bgp, i, j, alphabet, x_scale, colors, dat_idx, assay_data, K, unscaled_K)
    dat = assay_data[dat_idx]
    x_pde = dat.Position
    t_pde = dat.Time
    u_pde = dat.AvgDens
    ...
    err_CI = error_comp(bgp, pde_gp, x_pde, t_pde, u_pde)
    M = length(bgp.pde_setup.δt)
    GPAxis = Axis(fig[i, j], xlabel=L"$x$ (μm)", ylabel=L"u(x, t)/K", title=@sprintf("(%s): %i,000 cells per well. Error: (%.4g, %.4g)", alphabet, 10 + 2 * (dat_idx - 1), err_CI[1], err_CI[2]), titlealign=:left)
    @views for j in 1:M
        lines!(GPAxis, bgp.pde_setup.meshPoints * x_scale, soln_vals_mean[:, j] / x_scale^2 / unscaled_K, color=colors[j])
        band!(GPAxis, bgp.pde_setup.meshPoints * x_scale, soln_vals_upper[:, j] / x_scale^2 / unscaled_K, soln_vals_lower[:, j] / x_scale^2 / unscaled_K, color=(colors[j], 0.35))
        CairoMakie.scatter!(GPAxis, x_pde[t_pde.==bgp.pde_setup.δt[j]] * x_scale, u_pde[t_pde.==bgp.pde_setup.δt[j]] / x_scale^2 / unscaled_K, color=colors[j], markersize=3)
    end
    hlines!(GPAxis, K / x_scale^2 / unscaled_K, color=:black)
    ylims!(GPAxis, 0.0, 1.3)
    return nothing
end

function plot_pde_soln(bgp1, bgp2, bgp3, bgp4, bgp5, bgp6, x_scale, colors, assay_data, fontsize, filename, K, unscaled_K)
    fig = Figure(fontsize=fontsize, resolution=(1800, 1000))
    plot_idx = [(1, 1), (1, 2), (2, 1), (2, 2), (3, 1), (3, 2)]
    bgps = [bgp1, bgp2, bgp3, bgp4, bgp5, bgp6]
    alphabet = join('a':'z')
    [plot_pde_soln!(fig, bgps[i], plot_idx[i][1], plot_idx[i][2], alphabet[i], x_scale, colors, i, assay_data, K, unscaled_K) for i in 1:6]
