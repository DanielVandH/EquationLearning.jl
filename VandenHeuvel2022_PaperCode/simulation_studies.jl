...
densityFigures = Figure(fontsize=fontsize, resolution=(1800, 800))
...
densityFigures = Figure(fontsize=fontsize, resolution=(1200, 800))
diffusionDensityAxes = Vector{Axis}(undef, d)
reactionDensityAxes = Vector{Axis}(undef, r)
for i = 1:d
    diffusionDensityAxes[i] = Axis(densityFigures[1, i], xlabel=L"$\beta_%$i$ (μm²/h)", ylabel="Probability density",
...
bootstrap_setup = @set bootstrap_setup.show_losses = false
bootstrap_setup = @set bootstrap_setup.Optim_Restarts = 4
bgp2 = bootstrap_gp(x, t, u, T, D, D′, R, R′, α₀, β₀, γ₀, lowers, uppers; gp_setup, bootstrap_setup, optim_setup, pde_setup, D_params, R_params, T_params, verbose=false, zvals=bgp.zvals)
pde_gp = boot_pde_solve(bgp2, x_pde, t_pde, u_pde; ICType="gp")
...
D_params = [K, 300.0 * t_scale / x_scale^2, -200.0 * t_scale / x_scale^2]
R_params = [K, 0.044 * t_scale]
delay_scales1, diffusion_scales1, reaction_scales1 = nothing, [D_params[2] * x_scale^2 / t_scale, D_params[3] * x_scale^2 / t_scale], R_params[2] / t_scale
T_params = Vector{Float64}([])
D_params = [300.0] * t_scale / x_scale^2
R_params = [K, 0.044 * t_scale]
delay_scales2, diffusion_scales2, reaction_scales2 = nothing, D_params[1] * x_scale^2 / t_scale, R_params[2] / t_scale
delay_scales = [delay_scales1, delay_scales2]
diffusion_scales = [diffusion_scales1, diffusion_scales2]
reaction_scales = [reaction_scales1, reaction_scales2]
model_comparisons = AllResults(x_pde, t_pde, u_pde, bgp, bgp2; delay_scales, diffusion_scales, reaction_scales, x_scale, t_scale)
...
for i in 1:length(bgps_all)
    ...
    new_axis = Axis(fig[plot_positions[i][1], plot_positions[i][2]], xlabel=L"$x$ (μm)", ylabel=L"u(x, t)/K",
        ...
        titlealign=:left, width = 800, height = 350)
    @views for j in 1:M
...
Legend(fig[0, 1:2], [values(legendentries)...], [keys(legendentries)...], L"$t$ (h)", orientation=:horizontal, labelsize=34, titlesize=34, titleposition=:left)
